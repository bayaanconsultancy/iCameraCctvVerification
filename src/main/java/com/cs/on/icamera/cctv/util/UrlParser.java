...
public class UrlParser {
	...
	public static String addCredentialsToRtspUrl(String rtspUrl, String username, String password)
			throws OnvifException {
		try {
			URI uri = new URI(rtspUrl);
			String userInfo = username + ":" + password;
			return URLDecoder.decode(new URI(uri.getScheme(), userInfo, uri.getHost(), uri.getPort(), uri.getPath(),
					uri.getQuery(), uri.getFragment()).toString(), StandardCharsets.UTF_8);
		} catch (URISyntaxException e) {
