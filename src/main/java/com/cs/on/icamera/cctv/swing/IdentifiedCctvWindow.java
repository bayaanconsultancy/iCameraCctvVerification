...
public class IdentifiedCctvWindow {
    ...
    public IdentifiedCctvWindow() {
        ...
        discoveredCountLabel = new JLabel(String.valueOf(discoveredCount));
        discoveredCountLabel.setFont(discoveredCountLabel.getFont().deriveFont(Font.BOLD));
        gbc.gridx = 1;
        gbc.gridy = 0;
        frame.add(discoveredCountLabel, gbc);
        ...
        if (discoveredCount > 0) {
            ...
            JButton downloadTemplateButton = new JButton("Download Excel template");
    ...
    private void downloadExcelTemplate() {
        try {
            File file = FileDialogUtils.saveFileDialog("iCamera-cctv-verification-template");
            if (file != null) {
                Workbook.writeCctvsToExcel(file, SheetNames.TEMPLATE, TemplateColumns.values(), DataStore.getIdentifiedDevices());
            }
        } catch (Exception e) {
